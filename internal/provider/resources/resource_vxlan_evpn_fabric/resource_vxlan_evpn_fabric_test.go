...
func TestResourceVxlanEvpnFabric(t *testing.T) {
	type args struct {
		rscType  string
		rscName  string
		dataFile string
	...
	tests := []struct {
		...
		{
			name: "Test_resource_vxlan_evpn_fabric",
			args: args{
				rscType:  "resource",
				rscName:  "vxlan_evpn_fabric",
				dataFile: "/examples/ndfc_payloads/data_vxlan_evpn_fabric.json",
			},
		},
	}
