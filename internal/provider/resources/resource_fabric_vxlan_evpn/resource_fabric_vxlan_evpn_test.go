...
func TestResourceFabricVxlanEvpn(t *testing.T) {
	type args struct {
		rscType  string
		rscName  string
		dataFile string
	...
	tests := []struct {
		...
		{
			name: "Test_resource_fabric_vxlan_evpn",
			args: args{
				rscType:  "resource",
				rscName:  "fabric_vxlan_evpn",
				dataFile: "/examples/ndfc_payloads/data_fabric_vxlan_evpn.json",
			},
		},
	}
