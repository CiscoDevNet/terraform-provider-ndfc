...
func TestResourceInterfaceLoopback(t *testing.T) {
	type args struct {
		rscType  string
		rscName  string
		dataFile string
	...
	tests := []struct {
		...
		{
			name: "Test_resource_interface_loopback",
			args: args{
				rscType:  "resource",
				rscName:  "interface_loopback",
				dataFile: "/examples/ndfc_payloads/data_interface_loopback.json",
			},
		},
	}
