...
func (v *NDFCDhcpRelayServersValue) CreatePlan(c NDFCDhcpRelayServersValue, cf *bool) int {
	...
	if v.Address != c.Address {
		log.Printf("Address-Update: v.Address=%v, c.Address=%v", v.Address, c.Address)
	...
	if v.Vrf != c.Vrf {
		log.Printf("Vrf-Update: v.Vrf=%v, c.Vrf=%v", v.Vrf, c.Vrf)
		if action == ActionNone || action == RequiresUpdate {
			action = RequiresUpdate
		}
	}
...
func (v *NDFCNetworksValue) CreatePlan(c NDFCNetworksValue, cf *bool) int {
	...
	if v.DisplayName != c.DisplayName {
		log.Printf("DisplayName-Update: v.DisplayName=%v, c.DisplayName=%v", v.DisplayName, c.DisplayName)
		if action == ActionNone || action == RequiresUpdate {
			action = RequiresUpdate
		}
	}
	...
	if v.NetworkTemplate != c.NetworkTemplate {
		log.Printf("NetworkTemplate-Update: v.NetworkTemplate=%v, c.NetworkTemplate=%v", v.NetworkTemplate, c.NetworkTemplate)
		if action == ActionNone || action == RequiresUpdate {
			action = RequiresUpdate
		}
	}
	...
	if v.NetworkExtensionTemplate != c.NetworkExtensionTemplate {
		log.Printf("NetworkExtensionTemplate-Update: v.NetworkExtensionTemplate=%v, c.NetworkExtensionTemplate=%v", v.NetworkExtensionTemplate, c.NetworkExtensionTemplate)
		if action == ActionNone || action == RequiresUpdate {
			action = RequiresUpdate
		}
	}
	...
	if !v.PrimaryNetworkId.IsEmpty() && !c.PrimaryNetworkId.IsEmpty() {
		if *v.PrimaryNetworkId != *c.PrimaryNetworkId {
			log.Printf("PrimaryNetworkId-Update: v.PrimaryNetworkId=%v, c.PrimaryNetworkId=%v", *v.PrimaryNetworkId, *c.PrimaryNetworkId)
			if action == ActionNone || action == RequiresUpdate {
				action = RequiresUpdate
			}
		}
	...
	if v.NetworkType != c.NetworkType {
		log.Printf("NetworkType-Update: v.NetworkType=%v, c.NetworkType=%v", v.NetworkType, c.NetworkType)
		if action == ActionNone || action == RequiresUpdate {
			action = RequiresUpdate
		}
	}
	...
	if v.NetworkTemplateConfig.Layer2Only != "" {
		if v.NetworkTemplateConfig.Layer2Only != c.NetworkTemplateConfig.Layer2Only {
		...
		//v empty, fill with c
		log.Printf("Copy from state: v.NetworkTemplateConfig.Layer2Only=%v, c.NetworkTemplateConfig.Layer2Only=%v", v.NetworkTemplateConfig.Layer2Only, c.NetworkTemplateConfig.Layer2Only)
		v.NetworkTemplateConfig.Layer2Only = c.NetworkTemplateConfig.Layer2Only
	}
	...
	if v.NetworkTemplateConfig.ArpSuppression != "" {
		if v.NetworkTemplateConfig.ArpSuppression != c.NetworkTemplateConfig.ArpSuppression {
		...
		//v empty, fill with c
		log.Printf("Copy from state: v.NetworkTemplateConfig.ArpSuppression=%v, c.NetworkTemplateConfig.ArpSuppression=%v", v.NetworkTemplateConfig.ArpSuppression, c.NetworkTemplateConfig.ArpSuppression)
		v.NetworkTemplateConfig.ArpSuppression = c.NetworkTemplateConfig.ArpSuppression
	}
	...
	if v.NetworkTemplateConfig.IngressReplication != "" {
		...
		//v empty, fill with c
		log.Printf("Copy from state: v.NetworkTemplateConfig.IngressReplication=%v, c.NetworkTemplateConfig.IngressReplication=%v", v.NetworkTemplateConfig.IngressReplication, c.NetworkTemplateConfig.IngressReplication)
		v.NetworkTemplateConfig.IngressReplication = c.NetworkTemplateConfig.IngressReplication
	}
	...
	if len(v.NetworkTemplateConfig.DhcpRelayServers) != len(c.NetworkTemplateConfig.DhcpRelayServers) {
		log.Printf("DhcpRelayServers-Update: len(v.NetworkTemplateConfig.DhcpRelayServers)=%d, len(c.NetworkTemplateConfig.DhcpRelayServers)=%d", len(v.NetworkTemplateConfig.DhcpRelayServers), len(c.NetworkTemplateConfig.DhcpRelayServers))
		if action == ActionNone || action == RequiresUpdate {
			action = RequiresUpdate
		}
	}
	...
	if v.NetworkTemplateConfig.RouteTargetBoth != "" {
		...
		//v empty, fill with c
		log.Printf("Copy from state: v.NetworkTemplateConfig.RouteTargetBoth=%v, c.NetworkTemplateConfig.RouteTargetBoth=%v", v.NetworkTemplateConfig.RouteTargetBoth, c.NetworkTemplateConfig.RouteTargetBoth)
		v.NetworkTemplateConfig.RouteTargetBoth = c.NetworkTemplateConfig.RouteTargetBoth
	}
	...
	if v.NetworkTemplateConfig.Netflow != "" {
		...
		//v empty, fill with c
		log.Printf("Copy from state: v.NetworkTemplateConfig.Netflow=%v, c.NetworkTemplateConfig.Netflow=%v", v.NetworkTemplateConfig.Netflow, c.NetworkTemplateConfig.Netflow)
		v.NetworkTemplateConfig.Netflow = c.NetworkTemplateConfig.Netflow
	}
	...
	if v.NetworkTemplateConfig.L3GatwayBorder != "" {
		...
		//v empty, fill with c
		log.Printf("Copy from state: v.NetworkTemplateConfig.L3GatwayBorder=%v, c.NetworkTemplateConfig.L3GatwayBorder=%v", v.NetworkTemplateConfig.L3GatwayBorder, c.NetworkTemplateConfig.L3GatwayBorder)
		v.NetworkTemplateConfig.L3GatwayBorder = c.NetworkTemplateConfig.L3GatwayBorder
	}
