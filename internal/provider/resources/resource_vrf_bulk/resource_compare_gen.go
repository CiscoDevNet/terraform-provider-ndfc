...
func (v NDFCVrfsValue) DeepEqual(c NDFCVrfsValue) int {
	...
	if v.VrfName != c.VrfName {
		log.Printf("v.VrfName=%s, c.VrfName=%s", v.VrfName, c.VrfName)
		return RequiresReplace

	}

	if v.VrfTemplate != c.VrfTemplate {
		log.Printf("v.VrfTemplate=%s, c.VrfTemplate=%s", v.VrfTemplate, c.VrfTemplate)
		return RequiresUpdate

	}

	if v.VrfExtensionTemplate != c.VrfExtensionTemplate {
		log.Printf("v.VrfExtensionTemplate=%s, c.VrfExtensionTemplate=%s", v.VrfExtensionTemplate, c.VrfExtensionTemplate)
		return RequiresUpdate
