...
func TestResourceLanClassicFabric(t *testing.T) {
	type args struct {
		rscType  string
		rscName  string
		dataFile string
	...
	tests := []struct {
		...
		{
			name: "Test_resource_lan_classic_fabric",
			args: args{
				rscType:  "resource",
				rscName:  "lan_classic_fabric",
				dataFile: "/examples/ndfc_payloads/data_lan_classic_fabric.json",
			},
		},
	}
