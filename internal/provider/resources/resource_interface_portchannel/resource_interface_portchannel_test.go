...
func TestResourceInterfacePortchannel(t *testing.T) {
	type args struct {
		rscType  string
		rscName  string
		dataFile string
	...
	tests := []struct {
		...
		{
			name: "Test_resource_interface_portchannel",
			args: args{
				rscType:  "resource",
				rscName:  "interface_portchannel",
				dataFile: "/examples/ndfc_payloads/data_interface_portchannel.json",
			},
		},
	}
