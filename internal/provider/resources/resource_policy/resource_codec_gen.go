...
type NDFCPolicyModel struct {
	...
	SerialNumbers      CSVString         `json:"serialNumber,omitempty"`
	...
	ComputedParameters map[string]string `json:"computed_parameters,omitempty"`
}
...
func (v *PolicyModel) SetModelData(jsonData *NDFCPolicyModel) diag.Diagnostics {
	...
	if jsonData.GeneratedConfig != "" {
		v.GeneratedConfig = types.StringValue(jsonData.GeneratedConfig)
	} else {
		v.GeneratedConfig = types.StringNull()
	}

	if len(jsonData.ComputedParameters) == 0 {
		log.Printf("v.ComputedParameters is empty")
		v.ComputedParameters = types.MapNull(types.StringType)
	} else {
		mapData := make(map[string]attr.Value)
		for key, item := range jsonData.ComputedParameters {
			mapData[key] = types.StringValue(item)
		}
		v.ComputedParameters, err = types.MapValue(types.StringType, mapData)
		if err != nil {
			log.Printf("Error in converting map[string]string to  Map")
			return err
		}
...
func (v PolicyModel) GetModelData() *NDFCPolicyModel {
	...
	if !v.SerialNumbers.IsNull() && !v.SerialNumbers.IsUnknown() {
		...
		data.SerialNumbers = make(CSVString, len(listStringData))
