...
func TestResourceInterfaceEthernet(t *testing.T) {
	type args struct {
		rscType  string
		rscName  string
		dataFile string
	...
	tests := []struct {
		...
		{
			name: "Test_resource_interface_ethernet",
			args: args{
				rscType:  "resource",
				rscName:  "interface_ethernet",
				dataFile: "/examples/ndfc_payloads/data_interface_ethernet.json",
			},
		},
	}
