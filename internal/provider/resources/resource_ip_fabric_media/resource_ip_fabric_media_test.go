...
func TestResourceIpFabricMedia(t *testing.T) {
	type args struct {
		rscType  string
		rscName  string
		dataFile string
	...
	tests := []struct {
		...
		{
			name: "Test_resource_ip_fabric_media",
			args: args{
				rscType:  "resource",
				rscName:  "ip_fabric_media",
				dataFile: "/examples/ndfc_payloads/data_ip_fabric_media.json",
			},
		},
	}
