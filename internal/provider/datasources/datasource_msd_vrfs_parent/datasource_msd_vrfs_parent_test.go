...
func TestDatasourceMsdVrfsParent(t *testing.T) {
	type args struct {
		rscType  string
		rscName  string
		dataFile string
	...
	tests := []struct {
		...
		{
			name: "Test_datasource_msd_vrfs_parent",
			args: args{
				rscType:  "datasource",
				rscName:  "msd_vrfs_parent",
				dataFile: "/examples/ndfc_payloads/data_msd_vrfs_parent.json",
			},
		},
	}
	for _, tt := range tests {
		t.Run(tt.name+"model_read", func(t *testing.T) {
			...
			modelData := NDFCMsdVrfsParentModel{}
			v := MsdVrfsParentModel{}
