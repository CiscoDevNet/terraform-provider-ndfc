...
func (v VrfBulkModel) GetModelData() *NDFCVrfBulkModel {
	...
	if !v.Vrfs.IsNull() && !v.Vrfs.IsUnknown() {
		...
		for i, ele := range elements {
			if !ele.Id.IsNull() && !ele.Id.IsUnknown() {
				data.Vrfs[i].Id = new(int64)
				*data.Vrfs[i].Id = ele.Id.ValueInt64()
			...
			if !ele.FabricName.IsNull() && !ele.FabricName.IsUnknown() {
				data.Vrfs[i].FabricName = ele.FabricName.ValueString()
			} else {
				data.Vrfs[i].FabricName = ""
			}
