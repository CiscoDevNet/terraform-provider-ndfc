		...
		Steps: []resource.TestStep{
			{
				...
				Config: func() string {
					*stepCount++
					...
					helper.GenerateIntfResource(&intfRsc, 10, 10, "ethernet", true, helper.GetConfig().NDFC.Switches, true, false)
			...
			// Add some more intfs
			{
				Config: func() string {
					*stepCount++
					tName := fmt.Sprintf("%s_%d", t.Name(), *stepCount)
					helper.GenerateIntfResource(&intfRsc, 20, 10, "ethernet", true, helper.GetConfig().NDFC.Switches, true, true)
					(*x)["RscName"] = "eth_intf_test"
					helper.GetTFConfigWithSingleResource(tName, *x, []interface{}{intfRsc}, &tf_config)
					return *tf_config
				}(),
		...
		Steps: []resource.TestStep{
			{
				...
				Config: func() string {
					*stepCount++
					...
					helper.GenerateIntfResource(&intfRsc, 10, 20, "ethernet", true, helper.GetConfig().NDFC.Switches, true, false)
					(*x)["RscName"] = "eth_intf_test"
					helper.GetTFConfigWithSingleResource(tName, *x, []interface{}{intfRsc}, &tf_config)
					return *tf_config
				}(),
			...
			// Delete 10 interfaces
			{
				Config: func() string {
					*stepCount++
					tName := fmt.Sprintf("%s_%d", t.Name(), *stepCount)
					helper.GenerateIntfResource(&intfRsc, 10, -10, "ethernet", true, helper.GetConfig().NDFC.Switches, true, true)
					(*x)["RscName"] = "eth_intf_test"
					helper.GetTFConfigWithSingleResource(tName, *x, []interface{}{intfRsc}, &tf_config)
					return *tf_config
				}(),
				...
				Config: func() string {
					*stepCount++
					...
					helper.ModifyInterface(&intfRsc, 20, 5, "ethernet", map[string]interface{}{
						...
						"speed":       "Auto",
						"bpduGuard":   "false",
						"accessVlan":  int64(1000),
						"nativeVlan":  int64(1500),
					})
					(*x)["RscName"] = "eth_intf_test"
					helper.GetTFConfigWithSingleResource(tName, *x, []interface{}{intfRsc}, &tf_config)
					return *tf_config
			...
			},
			{
				Config: func() string {
					*stepCount++
					...
					helper.GenerateIntfResource(&intfRsc, 20, 10, "ethernet", true, helper.GetConfig().NDFC.Switches, true, true)
					//Delete 5
					helper.GenerateIntfResource(&intfRsc, 10, -5, "ethernet", true, helper.GetConfig().NDFC.Switches, true, true)
					//Modify 5
					helper.ModifyInterface(&intfRsc, 20, 5, "ethernet", map[string]interface{}{
						"admin_state":  "up",
						"mtu":          "jumbo",
						"speed":        "Auto",
						"bpduGuard":    "true",
						"accessVlan":   int64(1200),
						"nativeVlan":   int64(2400),
